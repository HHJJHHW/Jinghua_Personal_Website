    ...
    <div id="hobbies" className="py-12 md:py-32 px-4 md:px-6 max-w-7xl mx-auto">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20">
        ...
        <div className="lg:col-span-5 space-y-8 md:space-y-12">
          <div>
            <span className="text-duke-blue dark:text-blue-400 text-[10px] md:text-xs font-bold uppercase tracking-[0.2em] md:tracking-[0.3em] mb-3 md:mb-4 block">Personal Narrative</span>
            <h2 className="text-3xl md:text-5xl font-bold tracking-tighter-custom mb-4 md:mb-6 font-display dark:text-white">Symphony & Strategy</h2>
          ...
          <div className="group relative rounded-[2rem] md:rounded-[2.5rem] overflow-hidden bg-white dark:bg-slate-900 shadow-sm border border-slate-100 dark:border-slate-800 transition-all duration-700 hover:shadow-2xl hover:shadow-blue-900/5">
            <div 
            ...
            >
              ...
              <div className="absolute inset-0 bg-black/0 lg:group-hover:bg-black/10 transition-colors duration-500 flex items-center justify-center">
                <div className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-md p-3 md:p-4 rounded-full opacity-0 lg:group-hover:opacity-100 scale-90 lg:group-hover:scale-100 transition-all shadow-xl">
                  <ZoomIn size={20} className="text-duke-blue dark:text-blue-400" />
                </div>
              </div>
            </div>
            <div className="p-6 md:p-10">
              <div className="flex items-center gap-3 mb-3 md:mb-4">
                <div className="w-8 h-8 md:w-10 md:h-10 rounded-xl bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-duke-blue dark:text-blue-400">
                  <Music size={18} />
                </div>
                <h3 className="font-bold text-lg md:text-xl tracking-tight dark:text-slate-100">{HOBBIES.music.title}</h3>
              </div>
                ...
                {HOBBIES.music.images.map((_, i) => (
                  <button 
                    ...
                    className={`h-1 rounded-full transition-all duration-500 ${
                      i === activeMusicIdx ? 'w-6 bg-duke-blue dark:bg-blue-600' : 'w-1.5 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600'
                    }`} 
                  />
                ))}
              </div>
        ...
        <div className="lg:col-span-7">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 md:mb-12 gap-6">
            <h2 className="text-3xl md:text-5xl font-bold tracking-tighter-custom font-display dark:text-white">Visual Portfolio</h2>
            <div className="flex flex-wrap items-center gap-4 md:gap-6">
              <a 
                href={HOBBIES.photographyPortfolio} 
                target="_blank" 
                rel="noreferrer"
                className="flex items-center gap-2 text-duke-blue dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 font-bold text-[9px] md:text-[11px] uppercase tracking-widest transition-all bg-blue-50 dark:bg-blue-900/30 px-4 py-2 rounded-xl border border-blue-100 dark:border-blue-900/50"
              >
                <ExternalLink size={14} />
                <span>Full Portfolio</span>
              </a>
              <div className="flex items-center gap-2 text-slate-400 dark:text-slate-500 hover:text-duke-blue dark:hover:text-blue-400 cursor-pointer transition-colors">
                <Instagram size={16} />
          ...
          <div className="grid grid-cols-2 gap-4 md:gap-6">
            {HOBBIES.photography.map((photo, idx) => (
              <div 
                ...
                className={`group relative aspect-square rounded-2xl md:rounded-[2rem] overflow-hidden bg-slate-100 dark:bg-slate-800 cursor-zoom-in ${idx === 1 || idx === 2 ? 'lg:translate-y-12' : ''}`}
              >
                <img 
                  src={photo.url} 
                  alt={photo.caption} 
                  ...
                  loading="lazy"
                />
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 lg:group-hover:opacity-100 transition-all duration-500 flex flex-col justify-end p-4 md:p-8 text-left">
            ...
            {(lightbox.type === 'music' ? HOBBIES.music.images : HOBBIES.photography).map((item, i) => (
              <button 
                ...
                className={`w-12 h-12 md:w-20 md:h-20 flex-shrink-0 rounded-lg md:rounded-xl overflow-hidden border-2 transition-all transform ${
                  i === lightbox.index ? 'border-duke-blue dark:border-blue-500 scale-110 shadow-xl' : 'border-white/10 opacity-40 hover:opacity-80'
                }`}
              >
                <img 
